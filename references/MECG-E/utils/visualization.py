...
def generate_table(metrics, metric_values, Exp_names, dl_experiments_dict, label=None):
    ...
    for exp_name in Exp_names:
        ...
        for metric, metric_name in zip(metric_values, metrics):   # metric_values[metric][model][beat]
            m_mean = np.mean(metric[ind])
            m_std = np.std(metric[ind])
            tb_row.append('{:.3f}'.format(m_mean) + ' (' + '{:.3f}'.format(m_std) + ')')

            if label:
                dl_experiments_dict[exp_name][label][metric_name] = np.stack((m_mean, m_std))
            else:
                dl_experiments_dict[exp_name][metric_name]['average'] = np.stack((m_mean, m_std))
        tb.add_row(tb_row)
        ind += 1

    print(tb)
    return dl_experiments_dict
